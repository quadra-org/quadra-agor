# Register the `quadra_central_agor_mcp` MCP server (central instance)
...
```bash
claude mcp list   # → quadra_central_agor_mcp: https://agor.quadraplatform.com/mcp (HTTP) - ✔ Connected
```
...
- The tools are wrapped behind Agor's two-tier discovery: the agent calls
  ...
  scripting `claude -p --allowedTools`, allow `mcp__quadra_central_agor_mcp` (the server), not a bare
  tool name.
